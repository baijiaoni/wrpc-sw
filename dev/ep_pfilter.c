...
void pfilter_init_default()
{
	...
	pfilter_cmp(6, 0xcafe, 0xffff, MOV, 13);
  ...
  pp_printf("ETHERBONE NO P_FILTER!!!!!!!!!!!!!!!!!!!!!!!!!!!1 \n");
  ...
  pfilter_logic2(17, 13, AND, 3);
	pfilter_logic2(R_CLASS(7), 12, MOV, 0);	/* class 7: all non PTP and non-streamertraffic => external fabric */
	pfilter_logic2(R_CLASS(6), 11, MOV, 0); /* class 6: streamer broadcasts =>external fabric */
	pfilter_logic2(R_CLASS(0), 10, MOV, 0); /* class 0: PTP frames => LM32 */
	pfilter_logic2(R_CLASS(1), 17, MOV, 0);	/* class 0: ICMP/IP(unicast) or ARP(broadcast) or PTPv2 => PTP LM32 core */
  pp_printf("NO ETHERBONE !!!!!!!!!!!!!!!!!!!!!!!!!!!1 \n");
